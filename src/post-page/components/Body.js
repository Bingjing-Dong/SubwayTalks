...
function Body(props) {
  ...
  React.useEffect(() => {
    ...
    if (allFalse) {
      ...
      const stationsSelected = trainData.find(
        (trainObj) => trainObj.train === selectedTrain.name
      );
      setStatePost((prevState) => {
        return {
          ...prevState,
  ...
  const trainLogos = trains.map((train) => {
    return (
      <LogoTrain
        nameOfTrain={train.name}
        key={train.name}
        isHeld={train.isHeld}
        isOneHeld={IsOneSelected}
        holdStation={() => {
          holdStation(train.name);
        }}
        freeStation={() => {
          freeStation(train.name);
        }}
      />
    );
  });
  ...
  function handleSubmit(event) {
    ...
    axios
      ...
      .then(function (response) {
        ...
        stationUrl = response.data[0].url;
        stationTrains = response.data[0].trains;
        ...
        axios
          ...
          .then(function (response) {
            ...
            let runningSumDanger = 0;
            let runningSumStars = 0;
            for (let i = 0; i < response.data.length; i++) {
              runningSumDanger += Number(response.data[i].dangerLevel);
              runningSumStars += Number(response.data[i].overallRating);
            }
            runningSumDanger += Number(statePost.dangerLevel);
            runningSumStars += Number(statePost.satisfactionLevel);
              ...
              .then(function (response) {
                ...
                axios
                  ...
                  .then(function (response) {
                    ...
                    navigate(`/station/${statePost.station}`, {
                      state: {
                        station_id: stationId,
                        username: props.username,
                        userId: props.userId,
    ...
    <main className="main">
      ...
      <form onSubmit={handleSubmit}>
        <div>
          ...
          <div className="train-logos-sections-post-page">{trainLogos}</div>
        </div>
